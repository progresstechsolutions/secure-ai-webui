...
// Upload avatar
router.post('/avatar', extractUserInfo, upload.single('avatar'), async (req, res) => {
    try {
        if (!req.file) {
...
router.post('/files', extractUserInfo, upload.array('files', 5), async (req, res) => {
    try {
...
router.post('/images', extractUserInfo, upload.array('images', 5), async (req, res) => {
    try {
        if (!req.files || !Array.isArray(req.files) || req.files.length === 0) {
...
router.delete('/files/:filename', extractUserInfo, async (req, res) => {
    try {
