...
export class NotificationService {
     ...
     */
    static async createNotification({ recipient, sender, type, message, data = {} }) {
        try {
     ...
     */
    static async notifyPostLiked(postAuthor, liker, postId, postTitle) {
     ...
     */
    static async notifyPostComment(postAuthor, commenter, postId, postTitle, commentContent) {
        return this.createNotification({
            recipient: postAuthor,
            sender: commenter,
            type: 'post_comment',
     ...
     */
    static async notifyCommentReply(commentAuthor, replier, postId, commentId, replyContent) {
        return this.createNotification({
     ...
     */
    static async notifyCommunityInvite(invitee, inviter, communityId, communityName) {
        return this.createNotification({
            recipient: invitee,
            sender: inviter,
     ...
     */
    static async notifyJoinRequestResponse(requester, admin, communityId, communityName, accepted) {
        return this.createNotification({
            recipient: requester,
            sender: admin,
     ...
     */
    static async notifyNewMember(admin, newMember, communityId, communityName) {
        return this.createNotification({
            recipient: admin,
            sender: newMember,
            type: 'new_member',
    ...
    static async markAsRead(notificationId, userId) {
        try {
            const notification = await Notification.findOneAndUpdate({ _id: notificationId, 'recipient.id': userId }, { isRead: true }, { new: true });
            return notification;
        }
        catch (error) {
    ...
    static async markAllAsRead(userId) {
        try {
            const result = await Notification.updateMany({ 'recipient.id': userId, isRead: false }, { isRead: true });
            return result;
        }
        catch (error) {
