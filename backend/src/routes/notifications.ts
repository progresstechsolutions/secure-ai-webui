...
router.get('/', extractUserInfo, async (req: NotificationRequest, res: Response) => {
  try {
    const {
      page = '1',
      limit = '20',
...
router.patch('/:notificationId/read', extractUserInfo, async (req: AuthenticatedRequest<{ notificationId: string }>, res: Response) => {
  try {
...
router.patch('/mark-all-read', extractUserInfo, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'User not authenticated' });
      return;
...
// Get unread count
router.get('/unread-count', extractUserInfo, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'User not authenticated' });
      return;
...
// Delete notification
router.delete('/:notificationId', extractUserInfo, async (req: AuthenticatedRequest<{ notificationId: string }>, res: Response) => {
  try {
    const { notificationId } = req.params;
